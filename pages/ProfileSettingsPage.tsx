...
const ProfileSettingsPage: React.FC = () => {
    const { session, profile, createProfile, updateProfile, loading } = useAppContext();
    ...
    useEffect(() => {
        if (profile) { // Editando perfil existente
            setName(profile.name);
            setSpecialty(profile.specialty);
            setBio(profile.bio);
            setPhotoUrl(profile.photoUrl);
            setBookingSlotInterval(profile.bookingSlotInterval || 30);
        } else { // Criando novo perfil
            setName('');
            setSpecialty('');
            setBio(initialProfileData.bio);
            setPhotoUrl(initialProfileData.photoUrl);
            setBookingSlotInterval(initialProfileData.bookingSlotInterval);
        }
        setPhotoFile(null);
    }, [profile]);
    
    if (loading) {
        return <div className="text-center p-8"><Loader2 className="w-8 h-8 animate-spin text-primary mx-auto" /></div>;
    }
    ...
    const handleSave = async () => {
        ...
        try {
            let finalPhotoUrl = profile ? profile.photoUrl : photoUrl;
            ...
            if (profile) { // ATUALIZAR perfil existente
                await updateProfile({ name, specialty, bio, photoUrl: finalPhotoUrl, bookingSlotInterval });
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-gray-900">{profile ? 'Editar Perfil' : 'Crie Seu Perfil'}</h1>
                <p className="text-gray-500 mt-1">
                    {profile ? 'Atualize suas informações pessoais e de perfil.' : 'Preencha suas informações para que os clientes possam encontrar você.'}
                </p>
            </div>
            <div className="bg-white p-8 rounded-xl shadow-sm w-full max-w-2xl">
                <div className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="booking-interval" className="block text-sm font-medium text-gray-700">Intervalo entre horários (minutos)</label>
                        <select
                            id="booking-interval"
                            value={bookingSlotInterval}
                            onChange={e => setBookingSlotInterval(Number(e.target.value))}
                            className="mt-1 block w-full px-3 py-2 bg-white text-dark border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary"
                        >
                        ...
                        <p className="text-xs text-gray-500 mt-1">Define o espaçamento entre os horários disponíveis para agendamento. Ex: 9:00, 9:30, 10:00.</p>
                    </div>
                    <div>
                ...
                <div className="mt-8 flex justify-end gap-4 border-t pt-6">
                    <button onClick={() => navigate('/dashboard')} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors font-semibold">Cancelar</button>
